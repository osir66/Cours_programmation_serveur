...
class Cours(SQLModel, table=True):
    ...
    id_promo: int | None = Field(
        default=None, foreign_key="promotion.id_promo"
    )
    id_salle: int | None = Field(
        default=None, foreign_key="salle.id_salle"
    )

    id_prof: int | None = Field(
        default=None, foreign_key="prof.id_prof"
    )
